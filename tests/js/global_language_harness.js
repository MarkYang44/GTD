...
const global = boot({stored: "zh", globalStored: "en"});
assert.ok(global.globalApi, "shared API is exposed");
assert.strictEqual(global.globalApi.language, "en");
assert.strictEqual(global.globalApi.t("共 {n} 项", "{n} items", {n: 3}), "3 items");
global.globalApi.apply("zh", true);
assert.strictEqual(global.values.get("gtd_language_v1"), "zh");
global.windowListeners.storage({key: "gtd_language_v1", newValue: "en"});
assert.strictEqual(global.globalApi.language, "en");
