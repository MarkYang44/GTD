...
打开 `/kozekilmu/tracks`，可按赛道英文名称、中文别名或所在地搜索，并组合使用组别、车型和收藏筛选。搜索忽略大小写及重音符号。
...
**车型图鉴**（`/kozekilmu/cars`）独立展示现有数据中的全部车型、优点、注意事项和推荐它的赛道；推荐理由直接复用赛道指南。可按名称、组别、车型和收藏筛选，选 2–3 台并排比较。车型收藏与赛道指南共用浏览器存储，对比选择仅保留在当前页面。
...
每条推荐另有 **推荐来源与适用版本**。当前适用游戏版本均标为 **未验证**；来源核验日期只表示读过资料，不等于圈速或 BoP 验证。官方 V1.4.1.4 更新说明作为本次核对参考，历史驾驶指南可能不适用于现行物理版本。
