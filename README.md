...
计算方式：时长 ÷ 圈速向上取整，再加 1 圈；基础燃油 = 规划圈数 × 每圈油耗；安全余量取基础燃油的 10% 与 1 圈油耗中的较大值，总量向上取整至 0.1 L。全空时得到 **16 圈、52.8 L**。这是浏览器本地的通用估算，未模拟油箱容量、进站、编队圈、天气或节油；全场预算不等同于起步加油量，应按实际练习数据和赛事规则调整。
